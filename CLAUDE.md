...
2. **Offer the zero-setup path first**: `python3 app.py` with no keys set at
   all runs in **demo mode** on synthetic fixtures — no signup, and no
   network calls from any stage, *including* on a machine that already has
   `SEARXNG_URL` set (the statements stage is handed an explicit
   `statements.FORCE_OFFLINE`, not an empty string that used to fall
   through to the environment). Good enough to confirm the install works
   and show them what the tool does before they invest in real keys.
3. **For real data, two free keys, both instant self-service signups**:
   ...
   in `.env`, never in `run.sh` itself — see SECURITY.md.
...
| `parked/…` (not shipped) | **parked** | The old rhetoric-scoring + correlation track (`rhetoric.py`, `correlate.py`, a scored `statements.py`). Superseded by the factual design, never imported, and **not part of this repository** — it stays in the operator's working copy. Listed here only so a reference to it in an older note resolves. |
...
- **Candidate-generalized (this is done — an older note here said otherwise).**
  `build_candidate_context(name, office)` derives the official domain from the
  fixed congressional pattern (`<lastname>.senate.gov` / `.house.gov`) and the
  rhetoric actor from the resolved name + pronouns, so no per-candidate table
  is maintained. The module-level `_RHETORIC_RE` is the legacy Ossoff-or-"he"
  default and is only reached on the `ctx=None` path (standalone use and the
  older tests); every live run passes a ctx. See
  `tests/test_statements_classify.py`.

**Fixtures vs live — three distinct cases, and `""` is NOT `None`.**
`collect_statements(searxng_url=…)` reads: `None` (default) → consult
`SEARXNG_URL` in the environment; `FORCE_OFFLINE` (`""`) → offline fixtures,
no network, whatever the environment holds; any other string → that instance,
live. The old code was `base = (searxng_url or os.getenv("SEARXNG_URL", ""))`,
Python's falsy-string fallthrough, so app.py's demo branch passing `""` to
force fixtures fell through to the env instead: on any machine with SearXNG
configured, a demo run made **live web searches** and returned one real,
network-derived track sitting in an otherwise fabricated result — with the
step log stating "offline fixtures" (it described the branch's intent, not
the run). Reproduced live in the 2026-09-07 regression audit and fixed on
both halves: the sentinel, and a step line derived from `ss.backend`. Do not
collapse those three cases back into one `or`.
...
- **Never hard-code which key names a keyset-pagination cursor.** FEC's
  ...
  candidate, 100% reproducible, never disclosed (found by the
  5-candidate live rate-limit/completeness audit, 2026-09-07). Fixed by reading `last_indexes` back
...
- **`x or os.getenv(...)` is a trap wherever `""` is a meaningful value.**
  ...
  exactly how demo mode ran live web searches for months (see statements.py
  specifics). Where the difference matters, branch on `is None` and give the
  explicit case a named sentinel (`statements.FORCE_OFFLINE`).
- **A step-log line must be derived from what ran, not from the branch that
  ran it.** The demo statements step hardcoded "offline fixtures" and was
  therefore *wrong* in the one state where the collector did something else.
  Disclosures come from the returned object (`ss.backend`, `outspend.incomplete`,
  `rec.incomplete`), never from the intent of the code path — Integrity Rule 6
  is only worth anything if the log can't describe an intention.
...
- **Bind address defaults to `127.0.0.1` (post-2026-09-07 fix).** There's no
  ...
  (TLS + auth) in front first. See SECURITY.md.
...
- **`FLASK_DEBUG=1` + a non-loopback `HOST` is refused outright, not just
  warned about (post-2026-09-07 fix).** Individually, both defaults are
  safe (debug off, host loopback) — but an operator who sets BOTH env vars
  would otherwise get only a comment-and-a-warning between them and a live
  Werkzeug interactive debugger (arbitrary code execution) reachable by
  anyone who hits the port. The `__main__` block now checks this combo
  before calling `app.run()` and exits 1 with no bind attempted if both are
  set — see SECURITY.md.
- **`/health` no longer reports `has_fec_key`** (post-2026-09-07 fix) — it
  was pure reconnaissance value for an unauthenticated caller (distinguishing
  a real deployment from a demo one) with no legitimate consumer (grep
  `static/app.js` — nothing ever read it); the operator who wants to know
  already gets it from the startup console banner. `/health` is now a plain
  `{"ok": true}` liveness check, nothing more.
...
- **Launching:** `run.sh` is the launcher — it sources `.env` (if present;
  ...
  SECURITY.md; it used to have you paste keys directly into
...
- **Runtime modes:** DEMO runs the factual stages on synthetic fixtures with
  **no network calls at all** (synthesis skips in demo). It is triggered by an
  absent `FEC_API_KEY`, or by `"demo": true` in `/search`'s JSON body — **not**
  by a `?demo=1` query string. The app has never read `request.args` (grep it),
  and `static/app.js` never sends the body field, so the browser UI cannot
  reach demo mode on a keyed deployment even by accident; forcing it takes a
  hand-driven API call. The docs said `?demo=1` for a long time and the code
  never did — that mismatch was found by the 2026-09-07 regression audit and
  the docs were corrected to the code, not the other way round.
  REAL needs `FEC_API_KEY`;
...
- **Demo data says it is demo, everywhere it can be read.** `result["demo"]`
  (authoritative, run-level) rides through `/status` into the raw JSON pane and
  every export; `/status` also returns `demo` on its own so a poller knows
  before `result` exists; every demo track carries `_demo` at row level
  (`_mark_demo` in app.py — it used to be on only two of six tracks); and demo
  exports are named `demo_*.csv` / `demo_*.zip`. An exported file that has left
  the app must never be ambiguous about whether it is synthetic.
